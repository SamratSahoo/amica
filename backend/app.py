...
def add_file():
    ...
    webm_data = base64.b64decode(audio.replace("data:audio/x-m4a;base64,", ""))

    text = transcribe_text(webm_data)
    ...
    file_data = {"location": location, "timestamp": timestamp, "data": text}
...
def create_tasks(text: str, categories):
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"Your job is to take text which are real conversations from users and make tasks out of them that the user might need to complete. Make sure to return no tasks if there are none. All your responses should be JSON only return the list of tasks in the field 'tasks'. Also categorize tasks into one of the following categories : "
                + ", ".join(categories)
